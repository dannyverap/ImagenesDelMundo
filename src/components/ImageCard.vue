...
<template>
  ...
  >
    ...
    <div class="absolute bottom-0 left-0 right-0 p-3 flex flex-col">
      <div class="flex justify-end mt-auto">
        <button
          @click="seller?.id !== undefined && pickFavorite(seller.id)"
      ...
      <p
        class="bg-gray-700 mt-1 bg-opacity-50 rounded-lg text-sm font-semibold text-textSecondary p-1"
      >
        by: {{ seller?.name }}
      </p>
    </div>
  ...
  <ConfirmModal
    ...
    message="Are you sure you want to vote for this image as your favorite?"
    :isVisible="showModal"
