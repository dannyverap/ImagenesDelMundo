...
<template>
  <nav
    class="min-h-7vh bg-white text-textPrimary py-4 md:flex items-center justify-between px-6 shadow-md"
  >
    <div class="flex items-center">
      ...
      <ul class="hidden md:flex gap-5 ml-10">
        <li
          ...
          :key="idx"
          class="text-xl hover:text-hoverPrimary duration-300"
        >
          <RouterLink :to="{ name: tag.routeName }" class="hover:text-primaryDark">{{
            tag.name
          }}</RouterLink>
        </li>
      </ul>
    ...
    <div v-else class="flex items-center mt-2">
      ...
      <RouterLink
        :to="{ name: 'signup' }"
        class="hover:text-white bg-primary text-white px-4 py-2 rounded-lg"
      >
        Begin for Free
      </RouterLink>
    </div>
    <ul
      :class="[isNavVisible ? 'left-0' : 'left-[-100%]']"
      class="md:hidden bg-white w-full absolute top-14 left-0 px-6 pb-10 duration-500"
    >
      <li
        v-for="(tag, idx) in tags"
        :key="idx"
        class="my-6 text-xl hover:text-hoverPrimary duration-300"
      >
        <RouterLink :to="{ name: tag.routeName }" class="hover:text-primaryDark">{{
          tag.name
        }}</RouterLink>
